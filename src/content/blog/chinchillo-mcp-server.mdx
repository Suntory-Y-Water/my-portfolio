...
tags: 
  - MCP
description: この記事では簡単なMCPサーバーを作成していきます。
---

日進月歩でAIの進化が凄まじい日々を過ごしております。
ここ2週間くらいでMCPに関する記事がかなり増えた印象を持ちましたので、実際に手を動かしてどんなものか作成していきたいと思います。

いつも参考にしているブログで、サイコロを使った記事を発見しました。どうせなら少しアレンジしてみたいと思い、チンチロができるMCPサーバーを作ってみました。
...
| ------ | --- | ------------------------------------------------------------------------------- |
| ピンゾロ   | 5倍  | 1-1-1                                                                           |
| アラシ    | 3倍  | 1-1-1以外のゾロ目(2-2-2, 6-6-6など)両者ともにアラシのとき、数値が大きい人が勝つ
例: 2-2-2や6-6-6なら6-6-6が勝利      |
| シゴロ    | 2倍  | 4-5-6, 5-4-6など出目が4-5-6のとき                                                       |
| 通常役    | 1倍  | 1-1-2や6-6-1など同じで目が2個あるとき
両者ともに通常役のとき、重複していない数値が大きい人が勝つ
例: 1-1-2や6-6-1なら1-1-2が勝利 |
...
- Role: チンチロの出目と得点を定義する型
- DiceResult: サイコロを振った結果の型
- RollHistory: サイコロを降った結果を履歴として保持し、最終的な値を返却する型
...
テストが成功しました。最後に`index.ts`で`server.ts`で得た結果を実行します。
...
早速チンチロをやっていきましょう。
![https://pub-37c337e4f6b74be784982bc3041040b4.r2.dev/images/image.png-1744118301532.png](https://pub-37c337e4f6b74be784982bc3041040b4.r2.dev/images/image.png-1744118301532.png)
