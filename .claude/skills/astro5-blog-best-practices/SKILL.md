...
description: Astro 5.x静的ブログ構築のベストプラクティス集。Islands Architecture、SSG、Content Layer API、画像最適化、ビルド最適化、SEOなど、パフォーマンスとUXを最大化するための公式推奨手法を提供。Use when: (1) Astroでブログ機能を実装・修正する時、(2) Content Collectionsを設定する時、(3) 画像最適化やビルドパフォーマンスを改善する時、(4) クライアントディレクティブ(client:load/visible/idle)の選択が必要な時、(5) RSSフィードやサイトマップを実装する時。
---
