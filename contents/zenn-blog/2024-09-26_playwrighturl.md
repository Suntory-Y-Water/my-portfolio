...
# 確認方法
`response.url`の内容はChrome DevToolsのリクエストURLを見れば確認することができます。
...
使用例としてはこんな感じで、特定のページURL(この場合`inori-track.vercel.app/venue`)の通信だけをフィルタリングし、そこに紐づくクエリパラメータを`searchParams.getAll('hoge');`で取得します。
```tsx
  // 現在のURLを取得する
  page.on('response', async (response) => {
    // レスポンスURLを取得する
