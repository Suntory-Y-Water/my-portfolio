...
**注意**: 技術記事の場合、Tier 1レビューには**技術記事冒頭レビュー**が必須項目として含まれます。

#### 2-1. 初学者フィードバック

**エージェント呼び出し**:
```
Task tool (subagent_type: "beginner-feedback")
```
...
**エージェント呼び出し**:
```
Task tool (subagent_type: "blog-quality-review")
```
...
#### 2-3. AI警察レビュー（AIっぽさ検出）

**エージェント呼び出し**:
```
Task tool (subagent_type: "ai-police")
```

**評価項目**:
- Humanity Score（0.0-5.0点）
- 冗長表現検出数
- 過剰修飾検出数
- テンプレート表現検出数
- 不自然な丁寧語検出数
...
**出力**: Humanity Score、検出されたAI表現リスト、修正案、岡田翔太からのメッセージ

---

#### 2-4. 技術記事冒頭レビュー（技術記事の場合のみ・必須）⭐

**注意**: このレビューは**技術記事タイプの場合のみ**、Tier 1（必須）として実行します。コラム記事では実行しません。

**エージェント呼び出し**:
```
Task tool (subagent_type: "opening-context-reviewer")
```

**評価項目**:
- 時系列的背景の明示性（0.0-5.0点）
- 客観的データによる補強（0.0-5.0点）
- 因果関係の明確さ（0.0-5.0点）
- 冒頭3段落の構成バランス（0.0-5.0点）
...
**出力**: 総合評価、各項目の詳細評価、優先改善提案TOP 3、リライト例、石川拓海からのメッセージ

**レビュアー**: 石川拓海（いしかわ たくみ）・46歳、技術ブログ編集者 / ライティングコーチ

**重要性**: 技術記事の冒頭は「地図」です。読者が記事に足を踏み入れる前に、歴史的コンテキストと客観的データで全体像を示し、安心して進める道筋を提供します。
...
**エージェント呼び出し**:
```
Task tool (subagent_type: "narrative-architect")
```
...
**エージェント呼び出し**:
```
Task tool (subagent_type: "tech-beginner-feedback")
```
...
| Humanity Score（AI警察） | X.X/5.0 | [優秀/良好/要改善] |
| 冒頭レビュー（技術記事のみ） | X.X/5.0 | [優秀/良好/要改善] |
...
### 2-4. 技術記事冒頭レビュー（石川拓海・46歳）**技術記事の場合のみ**
...
- [ ] 素朴な疑問に回答できるか確認
- [ ] 品質スコアが3.5/5.0以上
- [ ] Humanity Score（AI警察）が3.0/5.0以上
- [ ] 致命的なAI臭さが修正済み
- [ ] **技術記事の場合**: 冒頭レビュースコアが3.5/5.0以上
- [ ] **技術記事の場合**: 時系列的背景が明示されている
- [ ] **技術記事の場合**: 客観的データで補強されている
- [ ] **技術記事の場合**: 因果関係が明確
- [ ] 承認した改善提案が実施済み
...
- `ai-police` - AIっぽさチェックのみ
- `opening-context-reviewer` - 技術記事冒頭レビューのみ（技術記事専用・Tier 1必須）
- `narrative-architect` - 物語性・共感性レビューのみ
- `title-specialist` - タイトル案生成

**コマンド**:
- `/punchline-review` - キーメッセージレビューのみ
- `/redundancy-check` - 冗長性チェックのみ
