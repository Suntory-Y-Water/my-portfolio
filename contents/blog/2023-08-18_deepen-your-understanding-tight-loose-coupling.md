...
3. **拡張性**: 新しい機能やコンポーネントを追加することが容易です。新しい実装をインターフェースに準拠させるだけで、既存のシステムと統合できます。
...
疎結合の例では、燃料の種類を**`LooseCar`**クラスの外部から取得できます。

また、**`LooseEngineA`** と **`LooseEngineB`** は異なる始動音を持っているので、それぞれのエンジンの実装を変更することなく、新しい始動音を追加できます。
