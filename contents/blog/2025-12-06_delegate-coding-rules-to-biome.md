...
例えば Stop Hooks では、Claude の処理自体が停止した後に、自動的に処理を動かすことができます。以下のようなフックは、コードの Write(書き込み)や Update(更新)、Edit(編集)などの作業が終わった後に、Biome などのフォーマッターを実行してフォーマットを整えるシェルスクリプトです。
```json
{
  "hooks": {
    "PostToolUse": [
...
このように、意図して Claude Code にコマンドを実行させるのではなく、自動的にコマンドを実行するような仕組みが、Claude Code には備わっています。
...
例えば、フォーマットのようにコマンドを実行するだけであれば先ほどのフックで実装できます。しかし TypeScript スクリプトで型をつけるときに `any` 型を使用しないことや `as` キャスト(アサーション)を使用しないといったルールは異なります。これらはどうしてもコードを実装する前や実装した後でしか解決できません。

アサーションレベルであれば `PreToolUse` といった実際に Claude が生成する前の行動を見て特定の単語が入っていないかを正規表現で判断可能です。しかしコード全体の話になると状況が変わります。例えば私の場合、「インターフェースよりも Type エイリアスを使って欲しい」や「引数が 2 個以上ある時に限ってオブジェクト形式にしてほしい」といった個人的なコーディングルールなどは守られないことが多いです。

Claude Code の書き方を指摘した記事、[Writing a good CLAUDE](https://www.humanlayer.dev/blog/writing-a-good-claude-md) では、 Claude Code は高級なリンターじゃないと指摘しています。もしコーディングルールを含めるようにするのであれば、Hooks などで強制的に検知させた方が良いと記載しています。

一般的にはコーディングするにあたっていくつかの規約があります。その 1 番最下部に属するものは、いわゆる静的解析と言われている ESLint や最近では Biome などによって機械的にパターンをルールでチェックするものです。これの上位に属するものが、単体テストや結合テスト、E2E テストといったテスト群ですが、コーディングルールの部分は、いわゆる一番下の静的解析の部分に当たる理解です。私は実務でも Biome を使用しておりますが、Biome v2.x から `GritQL` という機能が追加されました。これは簡単に説明すると、Biome 公式が提供していない独自のルールを設定することが可能になります。いわゆるオレオレルールが作成可能になったのです。

https://biomejs.dev/ja/linter/plugins/

## Biome v2.x で独自ルールの作成

GritQL スニペットはプロジェクト内のどこにでも配置できますが、 `.grit` 拡張子を使うことが推奨されています。
以下のように設定することで、プラグインを有効化できます。
...
今回作成するルールは「関数の引数が、2 個以上あるときにオブジェクト形式にする」です。引数が複数個設定可能な場合、呼び出し側で「２番目の引数ってなにが必要なんだっけ...」となってしまい、可読性が低下します。オブジェクト形式だった場合は、引数へ渡す順番を気にせずに済むのが大きなメリットです。それと、実際にどういうパラメーターを関数側が求めているかが明白だからです。

## Biome GritQLプラグインの実装

例えば、以下のようなソースコードの場合、name と age の 2 つの引数が設定されています。このようなソースコードの場合は、Biome 側で lint エラーになることが望ましいです。
```ts
...
これに対して Biome Lint を実行することでエラーになっていることが確認できます。画像でもわかる通り、VS Code のエディター上でも Biome 拡張機能が働いて赤い波線が設定されることが確認できます。
...
ここでのポイントは１つ目は可変長引数と同様にスプレッド構文を展開すれば、2 個以上 3 個以上といったルールを作成できます。２つめは関数に型注釈が設定されることもあるため、その場合も考慮して設定をします。
