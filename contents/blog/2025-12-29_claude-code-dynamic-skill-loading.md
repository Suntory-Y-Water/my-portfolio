...
この記事では、スキルの実行ロジックを理解し、スキルを確実に起動させる仕組みを考えていきます。
...
では、セマンティック類似度に頼らず、明示的にスキルを発動させる方法はないのでしょうか。
...
UserPromptSubmit Hooks は、ユーザーがプロンプトを送信した際、Claude Code が処理する前に実行されます。これにより、プロンプトや会話に基づいて追加のコンテキストを追加できます。また、プロンプトを検証したり、特定の種類のプロンプトをブロックすることも可能です。
