...
Cloudflare Browser Rendering は、ブラウザの自動化や Web スクレイピング、テスト、そしてコンテンツ生成のために、Cloudflare のグローバルネットワーク上でヘッドレス Chrome を実行できる機能です。主な用途としては、コンテンツのスクレイピングによってレンダリングされた HTML を抽出したり、Web ページを Markdown に変換したりといったことが挙げられます。
...
そもそも「後で読もう」とか「今読もう」となった時に、「あれ、この記事思ってたのと違うな…」とか「いや、これ見たことあるな」といった記事だった場合、拍子抜けしてしまうことがあります。本来あるべき姿は全てに目を通すことなんですが、例えば X (旧 Twitter) などで流れてきて「これは良さそうだな」と思い、今は別の作業中だから後で読もうとした時。あるいは Zenn やはてなブックマークなどでタイトルと冒頭だけ見て保存した時。そういった時に、後で最後まで読んでみるとがっかりする事態はなるべく避けたいなと思っています。

そこで、最低限何かしらの仕組みで要約してくれる方法はないかなと考えたところ、今回の Cloudflare による Markdown 変換機能が役に立つかもしれません。具体的には、実際に読みたいと思った記事の URL を送信してブラウザレンダリングで Markdown にした後、同じく Workers AI などを使って要約するサービスを作れば、事前にどういった記事が読めるのか（あるいは記事の概要）を知ることができるようになります。

実際に Safari などでページごとにコピーして ChatGPT などに投げる、という方法でも良いのですが、どうしても文章量が長かったり、そもそもブラウザのページをコピーしてペーストする作業自体が億劫だったりします。記事を探している時というのは、移動中やちょっとした休憩時間にスマホをちらっと見て「これは良さそうだから後で読もう」となることが多いです。最後に一気に読むためにも、その場で実施するタスクが増えるのは避けたいと思っています。ある程度半自動でやらないとこういった日々の習慣は定着しません。今回は Cloudflare の API を最大限活用して、実際に URL を送信し Markdown での出力ができるかを確認していきましょう。

Browser Rendering には、より高度な使い方として Puppeteer や Playwright を使った複雑なブラウザ操作や、MCP（Model Context Protocol）を活用したエージェントフローなども可能です。しかし、今回のように「URL を渡して Markdown を取得する」というシンプルな用途では、そこまでの機能は必要ありません。また、無料プランでは 10 分までの制限もありますし、静的ページの取得にわざわざ複雑なセットアップをするのも過剰だと思います。

今回は、シンプルに **[REST API](https://developers.cloudflare.com/browser-rendering/rest-api/)** の TypeScript SDK を使っていきます。
REST API は、スクリーンショットのキャプチャ、PDF の生成、HTML コンテンツの抽出など、シンプルでステートレスなタスクに最適です。「URL を渡して Markdown を取得する」という単一のタスクであれば、REST API で十分対応できます。
